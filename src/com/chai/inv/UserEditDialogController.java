...
public class UserEditDialogController {
	...
	@FXML private TextField x_FIRST_NAME;
	@FXML private TextField x_LAST_NAME;
	@FXML private TextField x_LOGIN_NAME;
	@FXML private PasswordField x_PASSWORD;
	@FXML private PasswordField x_CONFIRM_PASSWORD;
	@FXML private CheckBox x_STATUS;
	@FXML private CheckBox x_ACTIVATED;
	@FXML private TextField x_TELEPHONE_NUMBER;
	@FXML private TextField x_EMAIL;
	@FXML private DatePicker x_START_DATE;
	@FXML private DatePicker x_ACTIVATED_ON;
	@FXML private DatePicker x_END_DATE;
	@FXML private ComboBox<LabelValueBean> x_USER_TYPE_NAME; // LOGIN_TYPE
	@FXML private ComboBox<LabelValueBean> x_USER_ROLE;
	...
	private void initialize() {
		x_CONFIRM_PASSWORD.focusedProperty().addListener(new ChangeListener<Boolean>() {
			@Override
	...
	public void setUserBeanFields(UserBean selectedUserBean, LabelValueBean labelValueBean) {
		...
		if(DatabaseOperation.CONNECT_TO_SERVER && userBean.getX_LOGIN_NAME()!=null){
			if(userBean.getX_LOGIN_NAME().equals("cadmin")){
		...
		x_USER_ROLE.getItems().addAll(new LabelValueBean("----(select none)----",null));
		new SelectKeyComboBoxListener(x_USER_ROLE);
		if(userBean.getX_USER_ROLE_ID() != null){
			x_USER_ROLE.setValue(new LabelValueBean(userBean.getX_USER_ROLE_NAME(), userBean.getX_USER_ROLE_ID()));
		}
		x_USER_TYPE_NAME.setItems(userService.getDropdownList("TYPE"));
		x_USER_TYPE_NAME.getItems().addAll(new LabelValueBean("----(select none)----",null));
	...
	private void handleSubmitUser() throws SQLException {
		if (isValidate(actionBtnString)) {
			...
			if(x_USER_TYPE_NAME.getValue()!=null && !x_USER_TYPE_NAME.getValue().getLabel().equals("----(select none)----")){
			...
			if(actionBtnString.equals("search")){
				...
				if(userSaved && userRoleSaved){
			        ...
			        .owner(getDialogStage())
			        .title("Error")
			        .masthead("User record not saved")
			        .showError();
					dialogStage.close();
				}
				DatabaseOperation.getDbo().closeConnection();
	...
	public boolean isValidate(String actionBtnString) throws SQLException {
		...
		if (!actionBtnString.equals("search")) {
			...
			if (x_USER_TYPE_NAME.getValue() == null || x_USER_TYPE_NAME.getValue().toString().length() == 0 
					|| x_USER_TYPE_NAME.getValue().getLabel().equals("----(select none)----")) {
				errorMessage += "choose a login type!\n";
			}
			if (x_USER_ROLE.getValue() == null || x_USER_ROLE.getValue().toString().length() == 0 
					|| x_USER_ROLE.getValue().getLabel().equals("----(select none)----")) {
				errorMessage += "choose a user role!\n";
			}
			if(!actionBtnString.equals("edit")){
				if (x_PASSWORD.getText() == null || x_PASSWORD.getText().length() == 0) {
					errorMessage += "No valid password!\n";
				}
				if (x_CONFIRM_PASSWORD.getText() == null || x_CONFIRM_PASSWORD.getText().length() == 0) {
