	...
	public ObservableList<LabelValueBean> getDropdownList(String... action) {
		...
		switch(action[0]){
			case "item":
				...
				break;
			case "warehouse":
				x_QUERY = "SELECT WH.WAREHOUSE_ID, "
						+ "		  WH.WAREHOUSE_NAME "
...
//		dao = new DatabaseOperation();
		...
		try{
			...
			System.out.println("Error occured while inserting Miscellaneous "+transactionBean.getX_TRANSACTION_TYPE_CODE()+" data, error:"+ex.getMessage());
			setOperationMessage("Error occured while inserting Miscellaneous "+transactionBean.getX_TRANSACTION_TYPE_CODE()+" data, error:"+ex.getMessage());
			ex.getStackTrace();
			flag = false;
		}
		return flag;
...
//		dao = new DatabaseOperation();
		...
		try {
			...
			System.out.println("An error occured while Transaction search list, error:"
					+ ex.getMessage());
		} finally {
