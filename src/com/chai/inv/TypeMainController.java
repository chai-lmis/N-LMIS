...
//	private String actionBtnString;
	...
	private void initialize() {
		companyIdColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_COMPANY_ID"));
		typeIdColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_TYPE_ID"));
		typeCodeColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_TYPE_CODE"));
		typeNameColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_TYPE_NAME"));
		statusColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_STATUS"));
		decsriptionColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_TYPE_DESCRIPTION"));
		sourceTypeColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_SOURCE_TYPE"));
		startDateColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_START_DATE"));
		endDateColumn.setCellValueFactory(new PropertyValueFactory<TypeBean, String>("x_END_DATE"));
	...
	public boolean handleAddTypeAction() {
		...
		FXMLLoader loader = new FXMLLoader(MainApp.class.getResource("/com/chai/inv/view/AddType.fxml"));
		try {
			...
			controller.setTypeBeanFields(new TypeBean(), new LabelValueBean("Select Type Source", "0"));
	...
	public boolean handleEditAction() {
		...
		TypeBean selectedTypeBean = typeTable.getSelectionModel().getSelectedItem();
		if (selectedTypeBean != null) {
			LabelValueBean selectedLabelValueBean = new LabelValueBean(selectedTypeBean.getX_SOURCE_TYPE(),
					"",selectedTypeBean.getX_COMPANY_ID());
			FXMLLoader loader = new FXMLLoader(MainApp.class.getResource("/com/chai/inv/view/AddType.fxml"));
			try {
				AnchorPane typeAddEditDialog = (AnchorPane) loader.load();
				Stage dialogStage = new Stage();
				dialogStage.setTitle("Edit Type Form");
				dialogStage.initModality(Modality.WINDOW_MODAL);
				dialogStage.initOwner(primaryStage);
				Scene scene = new Scene(typeAddEditDialog);
				dialogStage.setScene(scene);
				...
				controller.setTypeBeanFields(selectedTypeBean, selectedLabelValueBean);
				controller.setTypeMain(this);
...
//				}
	...
	public boolean handleSearchAction() {
		...
		FXMLLoader loader = new FXMLLoader(MainApp.class.getResource("/com/chai/inv/view/AddType.fxml"));
		 try {
				 ...
				 controller.setTypeBeanFields(new TypeBean(), new LabelValueBean("Select Type", "0", ""));
				 controller.setTypeMain(this);				
				 dialogStage.showAndWait();
				 return controller.isOkClicked();
		 } catch (IOException e) {
	...
	public void handleExportAction() {
		...
		String csv = typeCodeColumn.getText()+","+typeNameColumn.getText()+","+decsriptionColumn.getText()+","
				+sourceTypeColumn.getText()+","+statusColumn.getText()+","+startDateColumn.getText()+","
				+ endDateColumn.getText();	
		String csvVal;
		for(TypeBean u : typeExportData){
			csv+="\n";
			csvVal = u.getX_TYPE_CODE()+","+u.getX_TYPE_NAME()+","+ u.getX_TYPE_DESCRIPTION()+","
					+ u.getX_SOURCE_TYPE()+","+ u.getX_STATUS()+","+u.getX_START_DATE()+","+u.getX_END_DATE();
        ...
        if (file != null) {
			...
			if (!file.getPath().endsWith(".xml") && !file.getPath().endsWith(".xlsx")&& !file.getPath().endsWith(".csv")){
