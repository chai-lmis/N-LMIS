...
public class SmartSearchComboBox<T> implements EventHandler<KeyEvent> {
	...
	public SmartSearchComboBox(ComboBox<T> comboBox) {
		this.comboBox = comboBox;
		this.comboBox.setOnKeyReleased(SmartSearchComboBox.this);
		this.comboBox.addEventFilter(KeyEvent.KEY_RELEASED,
		...
		this.comboBox.focusedProperty().addListener(new ChangeListener() {
			...
			public void changed(ObservableValue observable, Object oldValue,
				...
				if (newValue instanceof Boolean
					...
					ListView lv = ((ComboBoxListViewSkin) SmartSearchComboBox.this.comboBox
							.getSkin()).getListView();
		...
		for (int i = 0; i < items.size(); i++) {
			if (event.getCode() != KeyCode.BACK_SPACE
					&& items.get(i).toString().toLowerCase()
							.startsWith(sb.toString().toLowerCase())) {
				ListView lv = ((ComboBoxListViewSkin) comboBox.getSkin())
						.getListView();
