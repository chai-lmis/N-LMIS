...
//	private static ObservableList<AddOrderLineFormBean> list = FXCollections.observableArrayList();
	
	public static boolean validateOrderStatus(String actionBtnString, String orderStatus,
			ObservableList<AddOrderLineFormBean> list, Stage dialogStage, OrderFormController controller){
		boolean flag = true;
		...
		if(actionBtnString.equals("add") && orderStatus.equalsIgnoreCase("OPEN") && anyOrderLineSubmitted){
			...
			flag = false;
		}else if(actionBtnString.equals("add") && orderStatus.equalsIgnoreCase("SUBMITTED") && anyOrderLineOpen){
			masthead="Cannot place a new Order with different Order Status";
			message="One of your line product status is OPEN, it should be submitted if your order status is SUBMITTED";
			popUpDialog(masthead, message,dialogStage);
			flag = false;
		}
		if(actionBtnString.equals("edit") && orderStatus.equalsIgnoreCase("OPEN") && anyOrderLineSubmitted){
			...
			popUpDialog(masthead, message,dialogStage);
			flag = false;
		}else if(actionBtnString.equals("edit") && orderStatus.equalsIgnoreCase("OPEN") && allOrderLineCancel){
		...
		if(actionBtnString.equals("edit") && orderStatus.equalsIgnoreCase("SUBMITTED") && anyOrderLineOpen){
			masthead="Cannot Submit Order";
			message="One of your Order's Line item status is OPEN";
			popUpDialog(masthead, message,dialogStage);
			flag = false;
		}else if(actionBtnString.equals("edit") && orderStatus.equalsIgnoreCase("SUBMITTED") && allOrderLineCancel){
	...
	public static void anyOrderLineOpen(ObservableList<AddOrderLineFormBean> list){
	...
	public static void anyOrderLineSubmitted(ObservableList<AddOrderLineFormBean> list){
		for(AddOrderLineFormBean bean : list){			
	...
	public static void anySOOrderLineShipped(ObservableList<AddOrderLineFormBean> list){
		for(AddOrderLineFormBean bean : list){			
	...
	public static void anyOrderLineCancel(ObservableList<AddOrderLineFormBean> list){
		int cancelCount=0;
		for(AddOrderLineFormBean bean : list){
			...
			if(cancelCount==list.size()){
				...
				System.out.println("cancelcount < list.size - not all order Lines are cancel\n");
				allOrderLineCancel = false;
				allSOOrderLineCancel = false;
			}
		}
	}
	public static void popUpDialog(String masthead, String message, Stage dialogStage){
		Dialogs.create()
        .owner(dialogStage)
        .title("Information")
        .masthead(masthead)
	...
	public static boolean validateOrderStatus(String orderStatus, ObservableList<AddOrderLineFormBean> list, 
			Stage dialogStage, SalesOrderFormController controller){
		boolean flag = true;
		...
		if(orderStatus.equalsIgnoreCase("SHIPPED") && anySOOrderLineOpen){
			...
			popUpDialog(masthead, message,dialogStage);
			flag = false;
		}else if(orderStatus.equalsIgnoreCase("SHIPPED") && allSOOrderLineCancel){
			Action response = Dialogs.create()
			        .owner(new Stage())
			        .masthead("You cancelled all line products in your order.")
			        .message("Do you want to cancel complete order?")
