...
//			x_LINE_SHIP_QTY.setText(Integer.toString(sumOfLotQty));
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if (!newValue.booleanValue() && x_LINE_SHIP_QTY!=null && x_LINE_SHIP_QTY.getText()!=null) {
					...
					if(less && greater){						
						...
						String message=(less?"Ship Quantity cannot be less than or equal to Zero(0)":"Ship Quantity cannot be greater than Line Quantity");
						Dialogs.create()
						.owner(dialogStage)
						.title("Warning")
	...
	public void setFormDefaults(ObservableList<LabelValueBean> order_status_list,LabelValueBean orderStatusBean,
			String calledFrom, AddOrderLineFormBean addOrderLineFormBean,boolean activeReceiveLineStatus) {
...
//		x_LINE_UOM.getItems().addAll(new LabelValueBean("----(select none)----", null));
		if(calledFrom.equals("POOrderStatus")){
			...
			x_LINE_STATUS.setItems(orderFormService.getDropdownList(calledFrom));
			new SelectKeyComboBoxListener(x_LINE_STATUS);
		}
		if(orderStatusBean.getValue()==null){
			System.out.println("OrderFormcontroller : orderStatusBean is null");
		}else{
			System.out.println("OrderFormcontroller : orderStatusBean is not null,orderStatusBean.getValue(): "+orderStatusBean.getValue()+"\norderStatusBean.getLabel():"+orderStatusBean.getLabel());
		}
...
//			x_OK_BTN.setDisable(salesOrderFormController.getOK_BTN_DISABLE());
		...
		if(addOrderLineFormBean!=null){
			...
			x_LINE_ITEM.setValue(new LabelValueBean(addOrderLineFormBean.getX_LINE_ITEM(),
					addOrderLineFormBean.getX_LINE_ITEM_ID()));
			x_LINE_STATUS.setValue(new LabelValueBean(addOrderLineFormBean.getX_LINE_STATUS(),
					addOrderLineFormBean.getX_LINE_STATUS_ID()));			
			x_LINE_UOM.setValue(new LabelValueBean(addOrderLineFormBean.getX_LINE_UOM(),"no uom description")); //uom is auto-set when item was set
...
//		x_LINE_STATUS.setValue(lbv)[used to set order_line_status same as order_status by default whenever this form opens]
	...
	public void handleLineStatusChange() {
		...
		if (lbv.getLabel().equals("CANCEL")) {
			...
			System.out.println("In Line Status Change listener.... :cancel false :else block");
		...
		if (lbv.getLabel().equals("SHIPPED")) {
			...
			System.out.println("In Line Status Change listener.... :shipped false :else block");
			x_LINE_SHIP_QTY_LABEL.setVisible(false);
	...
	public void handleSetUomOnItemChng(){
		if(x_LINE_ITEM.getValue().getValue()!=null){
			x_LINE_UOM.setValue(new LabelValueBean(x_LINE_ITEM.getValue().getExtra1(),x_LINE_ITEM.getValue().getExtra1()));
		}		
	}
	...
	public boolean isValidate() {
		String errorMessage = "";
		if (x_LINE_ITEM.getValue() == null || x_LINE_ITEM.getValue().toString().length() == 0
		...
		if (x_LINE_UOM.getValue() == null || x_LINE_UOM.getValue().toString().length() == 0
				|| x_LINE_UOM.getValue().getLabel().equals("----(select none)----")) {
			errorMessage += "Select UOM\n";
		}
		if (x_LINE_STATUS.getValue() == null || x_LINE_STATUS.getValue().toString().length() == 0
				|| x_LINE_STATUS.getValue().getLabel().equals("----(select none)----")) {
			errorMessage += "Select Line Status\n";
		}
		if (x_LINE_STATUS.getValue() != null && x_LINE_STATUS.getValue().getLabel().equals("SHIPPED")) {
			if (x_LINE_SHIP_QTY.getText() == null || x_LINE_SHIP_QTY.getText().length() == 0) {
				errorMessage += "Ship Quantity cannot be left empty\n";			
			}else{
				try {
					int line_ship_qty = Integer.parseInt(x_LINE_SHIP_QTY.getText());
				} catch (Exception e) {
					errorMessage += "Must enter number for Ship-Quantity\n";
				}
			}
		}		
		if (x_LINE_STATUS.getValue() != null && x_LINE_STATUS.getValue().getLabel().equals("CANCEL")) {
			if (x_LINE_CANCEL_DATE.getValue() == null || x_LINE_CANCEL_DATE.getValue().toString().length() == 0) {
				errorMessage += "select Cancel-Date\n";
			}
			if (x_LINE_CANCEL_REASON.getText() == null || x_LINE_CANCEL_REASON.getText().length() == 0) {
	...
	public boolean handleAddLineItem() {
		...
		if (isValidate()) {
			...
			if(x_RECEIVED_QTY_HIDDEN_FIELD!=null && x_RECEIVED_QTY_HIDDEN_FIELD.getText()!=null && x_RECEIVED_QTY_HIDDEN_FIELD.getText().length()!=0){
			...
			if(x_LINE_CANCEL_DATE.getValue()!=null && x_LINE_CANCEL_DATE.getValue()!=null && 
			...
			if(edit_line_item){
				for(int i=0;i<list.size();i++){
					...
					if(aolb.getX_LINE_NUMBER().equals(addOrderLineFormBean.getX_LINE_NUMBER())){
						...
						if(x_LINE_CANCEL_DATE!=null && x_LINE_CANCEL_DATE.getValue()!=null && x_LINE_CANCEL_DATE.getValue().toString()!=null){
				...
				}else if(orderFormController!=null && aolb.getX_LINE_RECEIVED_DATE()!=null && 
	...
	public void setOrderLineItemGridList(ObservableList<AddOrderLineFormBean> list,
			String actionBtnString, String orderHeaderID){
		this.list = list;
		this.actionBtnString = actionBtnString;
	}
...
//			controller.setLineItemID(x_LINE_ITEM.getValue().getValue(),x_LINE_ITEM.getValue().getExtra1());
			...
			controller.setFormDefaults(orderFormService.getReceiveItemLotPopUp(addOrderLineFormBean.getX_ORDER_HEADER_ID(),
					x_LINE_ITEM.getValue().getValue()),addOrderLineFormBean,orderFormController.getX_SELECTED_STORE_NAME());
