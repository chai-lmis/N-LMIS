...
public class CustomerReceiveController implements LotReload {
	@FXML
	...
	public void handleOK() throws SQLException{
		if(isValid()){
			...
			transactionBean.setX_FROM_SOURCE("CUSTOMER");
			transactionBean.setX_TO_SOURCE("WAREHOUSE");

			transactionBean.setX_FROM_SOURCE_ID(x_CUSTOMERS.getValue().getValue());
			...
			transactionBean.setX_TRANSACTION_TYPE_CODE("CUSTOMER_RECEIPT");
			...
			if(transactionService.insertMiscReceiveRecord(transactionBean)){
				...
				.message("Health Facility Receipt Successful.. ")
				.showInformation();
				dialogStage.close();
	...
	public boolean isValid(){
		...
		if (x_CUSTOMERS.getValue() == null	|| x_CUSTOMERS.getValue().toString().length() == 0) {
			errorMessage += "No valid Customers selected!\n";
		}		
