...
public class CategoryMainController {
	...
	public void setRootLayoutController(RootLayoutController rootLayoutController) {
		...
		rootLayoutController.getX_ROOT_COMMON_LABEL().setText("Product Categories");
	}
	...
	private void initialize() {
		categoryIdColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_CATEGORY_ID"));
		companyIdColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_COMPANY_ID"));
		categoryCodeColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_CATEGORY_CODE"));
		categoryNameColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_CATEGORY_NAME"));
		decsriptionColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_CATEGORY_DESCRIPTION"));
		categoryTypeCodeColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_CATEGORY_TYPE_CODE"));
		categoryTypeIdColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_CATEGORY_TYPE_ID"));
		categoryTypeNameColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_CATEGORY_TYPE_NAME"));
		sourceCodeColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_SOURCE_CODE"));
		statusColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_STATUS"));
		startDateColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_START_DATE"));
		endDateColumn.setCellValueFactory(new PropertyValueFactory<CategoryBean, String>("x_END_DATE"));
	...
	public boolean handleAddCategoryAction() {
		...
		FXMLLoader loader = new FXMLLoader(MainApp.class.getResource("/com/chai/inv/view/AddCategory.fxml"));
		try {
			...
			controller.setCategoryBeanFields(new CategoryBean(),new LabelValueBean("Select Category Type", "0"));
	...
	public boolean handleEditAction() {
		...
		CategoryBean selectedCategoryBean = categoryTable.getSelectionModel().getSelectedItem();
		if (selectedCategoryBean != null) {
			LabelValueBean selectedLabelValueBean = new LabelValueBean(selectedCategoryBean.getX_CATEGORY_TYPE_NAME(),
			...
			try {
				...
				controller.setCategoryMain(this);
				controller.setUserBean(userBean);
				controller.setCategoryBeanFields(selectedCategoryBean,selectedLabelValueBean);
				// Show the dialog and wait until the user closes it
				dialogStage.showAndWait();
				return controller.isOkClicked();
			} catch (IOException e) {
	...
	@FXML void handleViewCategoryAction(){
	...
	public boolean handleSearchAction() {
		...
		FXMLLoader loader = new FXMLLoader(MainApp.class.getResource("/com/chai/inv/view/AddCategory.fxml"));
			 ...
			 controller.setCategoryBeanFields(new CategoryBean(), new LabelValueBean("Select Login Type", "0", ""));
			 controller.setCategoryMain(this);
			 controller.setUserBean(userBean);
			 // Show the dialog and wait until the user closes it
			 dialogStage.showAndWait();
	...
	public boolean handleHistoryAction() {
		...
		if (selectedCategoryBean != null) {
			...
			try {
				...
				historyBean.setX_PRIMARY_KEY(selectedCategoryBean.getX_CATEGORY_ID());
	...
	public void handleExportAction() {
		...
		ObservableList<CategoryBean> categoryExportData = categoryTable.getItems();
		String csv=categoryCodeColumn.getText()+","+categoryNameColumn.getText()+","
				+decsriptionColumn.getText()+","+categoryTypeNameColumn.getText()+","
				+sourceCodeColumn.getText()+","+categoryTypeCodeColumn.getText()+","
				+statusColumn.getText()+","+startDateColumn.getText()+","+endDateColumn.getText();				
		String csvVal;
		for(CategoryBean u : categoryExportData){
			csv+="\n";
			csvVal = u.getX_CATEGORY_CODE()+","+u.getX_CATEGORY_NAME()+","
						+ u.getX_CATEGORY_DESCRIPTION()+","+u.getX_CATEGORY_TYPE_NAME()+","
						+ u.getX_SOURCE_CODE()+","+u.getX_CATEGORY_TYPE_CODE()+","
