...
public class LotSubInvPopUpController {
	...
	@FXML private TextField x_TOTAL_SHIPPING_QTY;
				...
				public void handle(KeyEvent t) {
			    	...
			    	System.out.println("in textField.setOnKeyPressed...");
			    	 if(t.getCode().isDigitKey() || t.getCode()==KeyCode.BACK_SPACE){
				        ...
				        issueQty = Integer.parseInt(textField.getText().equals("")?"0":textField.getText());
				        System.out.println("lotOnhandQty="+lotOnhandQty);
				        System.out.println("issueQty="+issueQty);
				        
				        if(!(issueQty>=0 && issueQty<=Integer.parseInt(LINE_SHIP_QTY))){
...
//					            System.out.println("new issueQty = "+textField.getText());
						...
						if(!(sum>=0 && sum<=Integer.parseInt(LINE_SHIP_QTY))){
							Dialogs.create()
							.owner(new Stage())
							.title("Error")
							.message(" Sum of total quantity entered is greater than Ship quantity("+LINE_SHIP_QTY+")")
//							.message("Sum of all Entered Issue Quantity cannot be greater than Line's Ship Quantity("+LINE_SHIP_QTY+")")
	...
	public void setFormDefaults(ObservableList<TransactionBean> subList, ComboBox<LabelValueBean> TO_SOURCE){		
//		selectedRowIndex = new SalesOrderFormController().getSelectedRowIndex();
		this.subList = subList;
		this.LINE_SHIP_QTY = Integer.parseInt(addOrderLineFormBean.getX_LINE_SHIP_QTY());
		this.TO_SOURCE = TO_SOURCE;
		int rowIndex=1, arrayListIndex=0;
			for(TransactionBean bean : subList){
				if(bean.getX_ONHAND_QUANTITY()!=null & Integer.parseInt(bean.getX_ONHAND_QUANTITY())!=0){
					subinvTextObjList.add(new Text(bean.getX_FROM_SUBINVENTORY_CODE()));
					lotTextObjList.add(new Text(bean.getX_LOT_NUMBER()));
					expDateObjList.add(new Text(bean.getX_EXPIRATION_DATE()));
					onhandTextFieldObjList.add(new TextField(bean.getX_ONHAND_QUANTITY()));
					issueQtyTextFieldObjList.add(new TextField());		
					x_GRID_PANE.addRow(rowIndex,subinvTextObjList.get(arrayListIndex),lotTextObjList.get(arrayListIndex),
							expDateObjList.get(arrayListIndex),onhandTextFieldObjList.get(arrayListIndex),
							issueQtyTextFieldObjList.get(arrayListIndex));
					onhandTextFieldObjList.get(arrayListIndex).setDisable(true);	
					arrayListIndex++;
					rowIndex++;			
				}
			 setListenerOnIssueTextFields(addOrderLineFormBean.getX_LINE_SHIP_QTY());
			 textFieldIndex=0;
		}		
	}
	...
	public void handleOk() throws SQLException{
		for(TextField tf : issueQtyTextFieldObjList){
			sum+=(Integer.parseInt((tf.getText()==null) || (tf.getText().equals(""))?"0":tf.getText()));
		}System.out.println("issueQtyTextFieldObjList size = "+issueQtyTextFieldObjList.size()+", SUM="+sum);
		addOrderLineController.getX_LINE_SHIP_QTY().setText(Integer.toString(sum));
		if(!(sum>=0 && sum<=LINE_SHIP_QTY)){
			Dialogs.create()
			.owner(new Stage())
			.title("Error")
			.masthead(" Sum of total quantity entered is greater than Ship quantity("+LINE_SHIP_QTY+")")
//			.message("Sum of all Entered Issue Quantity cannot be greater than Line's Ship Quantity("+LINE_SHIP_QTY+")")
			.showWarning();
			sum = 0;
		}else{
			ObservableList<TransactionBean> list = FXCollections.observableArrayList();
			ObservableList<ReceiveLotSubinvPopUpBean> childLineItemList = FXCollections.observableArrayList();
			int i=0;
			for(TransactionBean bean : subList){
				System.out.println("subList.get(i).getX_LOT_NUMBER() = "+subList.get(i).getX_LOT_NUMBER());
				...
				if(i<issueQtyTextFieldObjList.size()){
					if(!issueQtyTextFieldObjList.get(i).getText().equals("")){
						System.out.println("Entered in loop's if block----------------=====");
						System.out.println("issueQtyTextFieldObjList.get(i).getText(): "+issueQtyTextFieldObjList.get(i).getText());
						System.out.println("transaction - -- - - bean");
						bean.setX_TRANSACTION_QUANTITY(issueQtyTextFieldObjList.get(i).getText().equals("")?"0":issueQtyTextFieldObjList.get(i).getText());
						System.out.println("bean.getX_TRANSACTION_QUANTITY(): "+bean.getX_TRANSACTION_QUANTITY());
						bean.setX_LOT_ISSUE_QUANTITY(bean.getX_TRANSACTION_QUANTITY());
						System.out.println("bean.getX_LOT_ISSUE_QUANTITY(): "+bean.getX_LOT_ISSUE_QUANTITY());
						//change it to TO_SOURCE.getValue().getValue()
						bean.setX_FROM_SOURCE_ID(userBean.getX_USER_WAREHOUSE_ID());
...
//						bean.setX_TO_SOURCE("WAREHOUSE");
						...
						System.out.println("addOrderLineFormBean.getX_LINE_ITEM_ID(): "+addOrderLineFormBean.getX_LINE_ITEM_ID());
						System.out.println("childBean.getITEM_ID(): "+childBean.getITEM_ID());
//							childBean.setITEM_NUMBER(bean.getX_ITEM_NUMBER());
						...
						System.out.println("childBean.getORDER_LINE_ID(): "+childBean.getORDER_LINE_ID());
						...
						System.out.println("childBean.getORDER_HEADER_ID(): "+childBean.getORDER_HEADER_ID());
						childBean.setLOT_NUMBER(bean.getX_LOT_NUMBER());
						System.out.println("childBean.getLOT_NUMBER(): "+childBean.getLOT_NUMBER());
						System.out.println("bean.getX_LOT_NUMBER(): "+bean.getX_LOT_NUMBER());
						childBean.setSHIP_QUANTITY(bean.getX_LOT_ISSUE_QUANTITY());
						System.out.println("bean.getX_LOT_ISSUE_QUANTITY(): "+bean.getX_LOT_ISSUE_QUANTITY());
						System.out.println("childBean.getSHIP_QUANTITY(): "+childBean.getSHIP_QUANTITY());
						...
						System.out.println("childBean.getSHIP_DATE(): "+childBean.getSHIP_DATE());
						
						childBean.setLINE_STATUS_ID(addOrderLineFormBean.getX_LINE_STATUS_ID());
						System.out.println("addOrderLineFormBean.getX_LINE_STATUS_ID(): "+addOrderLineFormBean.getX_LINE_STATUS_ID());
						System.out.println("childBean.getLINE_STATUS_ID(): "+childBean.getLINE_STATUS_ID());
						...
						System.out.println("childBean.getCANCEL_DATE(): "+childBean.getCANCEL_DATE());
						childBean.setCANCEL_REASON(addOrderLineFormBean.getX_LINE_CANCEL_REASON());
						System.out.println("addOrderLineFormBean.getX_LINE_CANCEL_REASON()"+addOrderLineFormBean.getX_LINE_CANCEL_REASON());
						System.out.println("childBean.setCANCEL_REASON(): "+childBean.getCANCEL_REASON());
						childBean.setUOM(addOrderLineFormBean.getX_LINE_UOM());
						System.out.println("addOrderLineFormBean.getX_LINE_UOM(): "+addOrderLineFormBean.getX_LINE_UOM());
						System.out.println("childBean.getUOM(): "+childBean.getUOM()+"\n\n");
						childBean.setSHIP_TO_WAREHOUSE_ID(TO_SOURCE.getValue().getValue());
						childBean.setSELF_LIFE(bean.getX_SELF_LIFE());
						childBean.setEXPIRATION_DATE(CalendarUtil.fromString(bean.getX_EXPIRATION_DATE()).toString());
						childBean.setMFG_OR_REC_DATE(CalendarUtil.fromString(bean.getX_MFG_OR_REC_DATE()).toString());
						childLineItemList.add(childBean);
						System.out.println("--------------------------exiting the loop's if blockkkk");
	...
	//bean 
	public void setAddOrderLineFormBean(AddOrderLineFormBean addOrderLineFormBean) {
