...
public class CustomerIssueController{
	@FXML
	private ComboBox<LabelValueBean> x_CUSTOMERS;
	@FXML
	...
	public void setFormDefaults(){
		...
		x_CUSTOMERS.setItems(transactionService.getDropdownList("customer"));
		new SelectKeyComboBoxListener(x_CUSTOMERS);
	...
	public void handleOK() throws SQLException{
		System.out.println("In CustomerIssueController.handleOk() method...");
		if(isValid()){
			transactionBean = new TransactionBean();
			...
			transactionBean.setX_TO_SOURCE("CUSTOMER");

			transactionBean.setX_FROM_SOURCE_ID(userBean.getX_USER_WAREHOUSE_ID());
			transactionBean.setX_TO_SOURCE_ID(x_CUSTOMERS.getValue().getValue());

			transactionBean.setX_FROM_SUBINVENTORY_ID(x_SUBINVENTORY.getValue().getValue());
			transactionBean.setX_TO_SUBINVENTORY_ID(null);

			transactionBean.setX_FROM_BIN_LOCATION_ID(x_LOCATOR.getValue().getValue());
			transactionBean.setX_TO_BIN_LOCATION_ID(null);

			transactionBean.setX_TRANSACTION_TYPE_CODE("CUSTOMER_ISSUE");
			...
			if(transactionService.insertMiscReceiveRecord(transactionBean)){
				...
				.message("Health Facility Issued Successfully.. ")
				.showInformation();
				dialogStage.close();
	...
	public boolean isValid(){
		System.out.println("In CustomerIssueController.isValid() method...");
		boolean flag = true;
		int onHandValue;
		int quantityValue;
		String errorMessage="";		
		
		if (x_CUSTOMERS.getValue() == null	|| x_CUSTOMERS.getValue().toString().length() == 0) {
			errorMessage += "No valid Health Facility selected!\n";
		}		
		if (x_ITEM.getValue() == null	|| x_ITEM.getValue().toString().length() == 0) {
			errorMessage += "No valid Product selected!\n";
		}
