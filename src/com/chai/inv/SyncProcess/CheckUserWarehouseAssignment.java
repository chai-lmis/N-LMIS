...
public class CheckUserWarehouseAssignment {
	...
	public static void insertUpdateTables(int warehouseId) {
		System.out
				.println("******************* User Warehouse Assignment Sync Started *********************");
			...
			if (localConn != null && serverConn != null) {
				...
				while (localRs.next()) {
					...
					sqlQuery = " SELECT COMPANY_ID, " // 1
							...
							+ " FROM ADM_USER_WAREHOUSE_ASSIGNMENTS "
							+ "WHERE USER_ID = "
							+ localRs.getString("USER_ID") // 13
							+ "  AND WAREHOUSE_ID = "
							+ localRs.getString("WAREHOUSE_ID"); // 14
					System.out
					...
					if (serverRs.next()) {
						...
						commonPStmt.setString(2,
								localRs.getString("WAREHOUSE_USER_ID"));
						...
						commonPStmt
								.setString(12, serverRs.getString("USER_ID"));
						...
						commonPStmt.setString(11,
								localRs.getString("LAST_UPDATED_ON"));
						System.out
								.println("commonPStmt :: Query to insert user_warehouse_assignment on Server ::"
										+ commonPStmt.toString());
						commonPStmt.executeUpdate();
						System.out
		...
		try {
			...
			if (localConn != null && serverConn != null) {
				...
				sqlQuery = " SELECT COMPANY_ID, " // 1
						...
						+ " WHERE USER_ID = "
						+ MainApp.getUserId()
						+ " AND SYNC_FLAG = 'N' AND WAREHOUSE_ID = "
						+ warehouseId;
				System.out
				...
				while (serverRs.next()) {
					...
					if (localRs.next()) {
						...
						commonPStmt.setString(2,
								serverRs.getString("WAREHOUSE_USER_ID"));
						commonPStmt.setString(3,
								serverRs.getString("WAREHOUSE_ID"));
					...
					System.out
							.println("Record is ready to update on warehouse !!!");
					sqlQuery = "UPDATE ADM_USER_WAREHOUSE_ASSIGNMENTS SET SYNC_FLAG='Y' WHERE USER_ID = "
							+ serverRs.getString("USER_ID")
							+ "   AND WAREHOUSE_ID =  "
							+ serverRs.getString("WAREHOUSE_ID");
					commonPStmt = serverConn.prepareStatement(sqlQuery);
