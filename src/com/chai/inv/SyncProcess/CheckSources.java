...
public class CheckSources {
	...
	public static void insertUpdateTables() {
		System.out
				.println("******************* Sources table Sync Started *********************");
				...
				while (serverRs.next()) {
					...
					if (localRs.next()) {
						...
						commonPStmt.setString(3,
								serverRs.getString("SOURCE_NAME"));
						commonPStmt.setString(4,
								serverRs.getString("SOURCE_DESCRIPTION"));
					...
					System.out
							.println("sync_flag Record is ready to update on server !!!");
					sqlQuery = "UPDATE SOURCES SET SYNC_FLAG='Y' WHERE SOURCE_CODE = '"
							+ serverRs.getString("SOURCE_CODE") + "'";
