...
public class CheckInventoryWarehouse {
	...
	public static void insertUpdateTables(int warehouseId) {
		System.out
				.println("******************* Check INVENTORY WAREHOUSES Started *********************");
		...
		try {
			...
			if (localConn != null && serverConn != null) {
				...
				while (serverRs.next()) {
					...
					commonPStmt
							.setString(2, serverRs.getString("WAREHOUSE_ID"));
					commonPStmt.setString(3,
							serverRs.getString("WAREHOUSE_CODE"));
					commonPStmt.setString(4,
							serverRs.getString("WAREHOUSE_NAME"));
					commonPStmt.setString(5,
							serverRs.getString("WAREHOUSE_DESCRIPTION"));
					commonPStmt.setString(6,
							serverRs.getString("WAREHOUSE_TYPE_ID"));
					...
					commonPStmt.setString(12,
							serverRs.getString("TELEPHONE_NUMBER"));
					...
					System.out
							.println("LGA Record updated successfully on local DB.....");
					sqlQuery = "SELECT WAREHOUSE_ID, COMPANY_ID, WAREHOUSE_CODE, WAREHOUSE_NAME, WAREHOUSE_DESCRIPTION, WAREHOUSE_TYPE_ID,"
							+ "ADDRESS1, ADDRESS2, ADDRESS3, STATE_ID, COUNTRY_ID, TELEPHONE_NUMBER, FAX_NUMBER, STATUS,"
							+ "START_DATE, END_DATE, CREATED_BY, CREATED_ON, UPDATED_BY, LAST_UPDATED_ON, REFERENCE_ID, DEFAULT_ORDERING_WAREHOUSE_ID,"
							+ "REFERENCE,TOTAL_POPULATION,YEARLY_PREGNANT_WOMEN_TP,MONTHLY_PREGNANT_WOMEN_TP,"
							...
							+ "WHERE WAREHOUSE_ID = "
							+ serverRs
									.getString("DEFAULT_ORDERING_WAREHOUSE_ID");
					serverPStmt = serverConn.prepareStatement(sqlQuery);
					// serverRs now contain state store record.
					serverRs = serverPStmt.executeQuery();
					serverRs.next();
					sqlQuery = "SELECT WAREHOUSE_ID FROM INVENTORY_WAREHOUSES WHERE WAREHOUSE_ID = "
							+ serverRs.getString("WAREHOUSE_ID");
					System.out
							.println("Query to check whether state store data need to be insert/update on warehouse :: "
									+ sqlQuery);
					...
					if (localRs.next()) {
						...
						commonPStmt.setString(6,
								serverRs.getString("WAREHOUSE_TYPE_ID"));
						commonPStmt
								.setString(7, serverRs.getString("ADDRESS1"));
						commonPStmt
								.setString(8, serverRs.getString("ADDRESS2"));
						commonPStmt
								.setString(9, serverRs.getString("ADDRESS3"));
						// commonPStmt.setString(10,
						// serverRs.getString("CITY_ID"));
						commonPStmt.setString(10,
						...
						commonPStmt.setString(21,
								serverRs.getString("REFERENCE_ID"));
						commonPStmt.setString(22, serverRs
								.getString("DEFAULT_ORDERING_WAREHOUSE_ID"));
						commonPStmt.setString(23, serverRs.getString("REFERENCE"));
						commonPStmt.setString(24, serverRs.getString("TOTAL_POPULATION"));
						commonPStmt.setString(25, serverRs.getString("YEARLY_PREGNANT_WOMEN_TP"));
						...
						commonPStmt.executeUpdate();
						System.out
								.println("LGA's State Store Record updated successfully on local DB.....");
					} else {
						System.out
						...
						commonPStmt.executeUpdate();
						System.out
								.println("LGA's State Store Record inserted successfully on warehouse.....");
					}
				}
				dbm.commit();
			} else {
