...
public class CheckCustomerProductConsumption {
	...
	public static void insertUpdateTables(int warehouseId) {
		System.out
				.println("******************* Check Customer Product Consumptions Started *********************");
		...
		try {
			...
			if (localConn != null && serverConn != null) {
				...
				while (serverRs.next()) {
					...
					System.out
							.println("Query to check whether the data need to be insert on warehouse :: "
									+ sqlQuery);
					...
					if (!localRs.next()) {
						...
						commonPStmt.setString(8,
								serverRs.getString("ORDER_CREATED_FLAG"));
						commonPStmt.setString(9,
								serverRs.getString("ALLOCATION_TYPE"));
						...
						System.out
								.println("Sync flag: Record is ready to update on warehouse !!!");
						sqlQuery = "UPDATE CUSTOMER_PRODUCT_CONSUMPTION SET "
								...
								+ "  AND WAREHOUSE_ID =  "+ serverRs.getString("WAREHOUSE_ID");
						System.out
								.println("Query to update CUSTOMER_PRODUCT_CONSUMPTION on SERVER :: "
										+ sqlQuery);
				...
				while (localRs.next()) {
					...
					if (!serverRs.next()) {
						...
						sqlQuery = "INSERT INTO CUSTOMER_PRODUCT_CONSUMPTION"
								...
								+ " VALUES (?,?,?,?,?,?,?,?,?)";
						System.out
								.println("Query to insert customer product consumption on Server :: "
										+ sqlQuery);
						commonPStmt = serverConn.prepareStatement(sqlQuery);
						commonPStmt.setString(1,
						...
						commonPStmt.setString(8,
								localRs.getString("ORDER_CREATED_FLAG"));
						commonPStmt.setString(9,
								localRs.getString("ALLOCATION_TYPE"));
						System.out.println("commonPStmt :: "
								+ commonPStmt.toString());
						commonPStmt.executeUpdate();
						...
						sqlQuery = "UPDATE CUSTOMER_PRODUCT_CONSUMPTION SET "
								+ " SYNC_FLAG='Y' " + "WHERE CONSUMPTION_ID = "
								+ localRs.getString("CONSUMPTION_ID") + " "
								+ "  AND WAREHOUSE_ID =  "
								+ localRs.getString("WAREHOUSE_ID")
								+ " AND CUSTOMER_ID = "+localRs.getString("CUSTOMER_ID");
						System.out
								.println("Query to update CUSTOMER_PRODUCT_CONSUMPTION on local warehouse :: "
										+ sqlQuery);
						commonPStmt = localConn.prepareStatement(sqlQuery);
						commonPStmt.executeUpdate();
						System.out
