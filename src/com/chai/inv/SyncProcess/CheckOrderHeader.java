...
public class CheckOrderHeader {
	...
	public static void insertUpdateTables(int warehouseId) {
		System.out
				.println("******************* Check Order Header Started *********************");
		...
		try {
			...
			if (localConn != null && serverConn != null) {
				...
				sqlQuery = "SELECT ORDER_HEADER_ID, ORDER_HEADER_NUMBER, ORDER_DATE, ORDER_STATUS_ID, EXPECTED_DATE, "
						...
						+ "FROM ORDER_HEADERS " + "WHERE SYNC_FLAG = 'N' ";
				System.out
				...
				while (localRs.next()) {
					System.out
							.println("****Checking whether the data is available on Order Line or Not*****");
					sqlQuery = " SELECT * " + " FROM ORDER_LINES "
					...
					if (commonRs.next()) {
						System.out
								.println("***Record available on Order Lines*****");
						System.out
						...
						if (serverRs.next()) {
							System.out
									.println("Record available, Need to update on server, Order # :: "
											+ localRs
							...
							System.out
									.println("...Record not available, Need to insert, Order # :: "
											+ localRs
													.getString("ORDER_HEADER_NUMBER"));
							...
							commonPStmt.setString(2,
									localRs.getString("ORDER_HEADER_NUMBER"));
							commonPStmt.setString(3,
									localRs.getString("ORDER_DATE"));
							commonPStmt.setString(4,
									localRs.getString("ORDER_STATUS_ID"));
							commonPStmt.setString(5,
									localRs.getString("EXPECTED_DATE"));
							commonPStmt.setString(6,
									localRs.getString("SHIP_DATE"));
							commonPStmt.setString(7,
									localRs.getString("ORDER_FROM_SOURCE"));
							commonPStmt.setString(8,
									localRs.getString("ORDER_FROM_ID"));
							commonPStmt.setString(9,
									localRs.getString("ORDER_TO_SOURCE"));
							commonPStmt.setString(10,
									localRs.getString("ORDER_TO_ID"));
						...
						if (updateFlag) {
							...
							sqlQuery = "UPDATE ORDER_HEADERS SET "
									...
									+ localRs.getString("ORDER_HEADER_ID");
							System.out
									.println("Query to update order headers on warehouse :: "
											+ sqlQuery);
							commonPStmt = localConn.prepareStatement(sqlQuery);
							commonPStmt.executeUpdate();
							System.out
									.println("Record updated successfully on warehouse for Order # :: "
											+ localRs
													.getString("ORDER_HEADER_NUMBER"));
						}
					} else {
						System.out
								.println("***Record not available on Order Lines*****");
					}
				}
				dbm.commit();
			} else {
		...
		try {
			...
			if (localConn != null && serverConn != null) {
				...
				while (serverRs.next()) {
					...
					localRs = localPStmt.executeQuery();
					if (localRs.next()) {
						System.out.println("Record available, Need to update on warehouse, Order # :: "
										+ serverRs.getString("ORDER_HEADER_NUMBER"));
						if (CheckData.updateCheckFromServer) {
							sqlQuery = "UPDATE ORDER_HEADERS SET "
									...
									+ " SHIPPED_DATE_ON_RECEIVE=? "// 23
									+ " WHERE ORDER_HEADER_ID = ?";// 24
							System.out.println("Query to update order headers on Server :: "
											+ sqlQuery);
							commonPStmt = localConn.prepareStatement(sqlQuery);
							commonPStmt.setString(1,
									serverRs.getString("ORDER_HEADER_NUMBER"));
							commonPStmt.setString(2,
									serverRs.getString("ORDER_DATE"));
							commonPStmt.setString(3,
									serverRs.getString("ORDER_TO_ID"));
							commonPStmt.setString(4,
									serverRs.getString("ORDER_TO_SOURCE"));
							commonPStmt.setString(5,
									serverRs.getString("ORDER_FROM_ID"));
							commonPStmt.setString(6,
									serverRs.getString("ORDER_FROM_SOURCE"));
							commonPStmt.setString(7,
									serverRs.getString("EXPECTED_DATE"));
							commonPStmt.setString(8,
									serverRs.getString("ORDER_STATUS_ID"));
							commonPStmt.setString(9,
									serverRs.getString("ORDER_TYPE_ID"));
							commonPStmt.setString(10,
									serverRs.getString("REFERENCE_ORDER_ID"));
							commonPStmt.setString(11,
									serverRs.getString("COMMENT"));
							commonPStmt.setString(12,
									serverRs.getString("CANCEL_DATE"));
							commonPStmt.setString(13,
									serverRs.getString("CANCEL_REASON"));
							commonPStmt.setString(14,
									serverRs.getString("STATUS"));
							commonPStmt.setString(15,
							...
							commonPStmt.setString(21, "Y");
							commonPStmt.setString(22,
									"SYNC_PROCESS_NEW_UPDATE_STOCK_ORDER");
							commonPStmt.setString(23, 
									serverRs.getString("SHIPPED_DATE_ON_RECEIVE"));
							commonPStmt.setString(24,
							...
							System.out
									.println("*******Order Line Not Updated Yet*********");
							updateFlag = false;
						}
					} else {
						System.out.println("Record not available, Need to insert, Order # :: "
										+ serverRs.getString("ORDER_HEADER_NUMBER"));
						sqlQuery = "INSERT INTO ORDER_HEADERS(ORDER_HEADER_ID,ORDER_HEADER_NUMBER, ORDER_DATE, ORDER_STATUS_ID, "
								+ "EXPECTED_DATE, SHIP_DATE, ORDER_FROM_SOURCE, ORDER_FROM_ID, ORDER_TO_SOURCE, "
								+ "ORDER_TO_ID, CANCEL_DATE, CANCEL_REASON, STATUS, START_DATE, END_DATE, "
						...
						commonPStmt.setString(23, "Y");
						commonPStmt.setString(24,
								"SYNC_PROCESS_NEW_INSERT_ORDER_FULFIL");
						...
						System.out.println("commonPStmt :: "
								+ commonPStmt.toString());
						commonPStmt.executeUpdate();
						System.out.println("Record inserted successfully on warehouse for Order # :: "
										+ serverRs.getString("ORDER_HEADER_NUMBER"));
					}
					if (updateFlag) {
						...
						commonPStmt = serverConn.prepareStatement(sqlQuery);
						commonPStmt.executeUpdate();
						System.out.println("Record updated successfully for Order # :: "
										+ serverRs.getString("ORDER_HEADER_NUMBER"));
					}
				}
				dbm.commit();
