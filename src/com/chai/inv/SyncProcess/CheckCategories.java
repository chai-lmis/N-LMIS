...
public class CheckCategories {
	...
	public static void insertUpdateTables(int warehouseId, Connection localConn, Connection serverConn) {
		...
		System.out.println("******************* Check CATEGORIES Started *********************");
		try {
			if (localConn != null && serverConn != null) {
//				dbm.setAutoCommit();
				System.out.println(".................CATEGORIES - STEP2 Checking whesther any data available on server to be sync .................");
				...
				while (serverRs.next()) {
					...
					System.out.println("CATEGORIES - STEP2 Data availbale to sync on SERVER");
					sqlQuery = "SELECT COMPANY_ID, CATEGORY_ID, CATEGORY_CODE, CATEGORY_NAME, CATEGORY_DESCRIPTION, SOURCE_CODE, "
							+ "CATEGORY_TYPE_ID, STATUS, START_DATE, END_DATE, CREATED_BY, CREATED_ON, UPDATED_BY, LAST_UPDATED_ON "
							+ "FROM CATEGORIES "
							+ "WHERE CATEGORY_ID = "
							+ serverRs.getString("CATEGORY_ID");
					System.out.println("CATEGORIES - STEP2 Query to check whether the data need to be insert or update on LOCAL DB :: "+ sqlQuery);
					localPStmt = localConn.prepareStatement(sqlQuery);
					localRs = localPStmt.executeQuery();
					if (!localRs.next()) {
						System.out.println("CATEGORIES - STEP2 Record not available, Need to insert on LOCAL DB.");
						...
						commonPStmt.setString(2,serverRs.getString("CATEGORY_ID"));
						commonPStmt.setString(3,serverRs.getString("CATEGORY_CODE"));
						commonPStmt.setString(4,serverRs.getString("CATEGORY_NAME"));
						commonPStmt.setString(5,serverRs.getString("CATEGORY_DESCRIPTION"));
						commonPStmt.setString(6,serverRs.getString("SOURCE_CODE"));
						commonPStmt.setString(7,serverRs.getString("CATEGORY_TYPE_ID"));
						...
						System.out.println("CATEGORIES - STEP2 Record inserted successfully on LOCAL DB.");
					}
				}
//				dbm.commit();
			} else {
