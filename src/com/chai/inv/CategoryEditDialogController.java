...
public class CategoryEditDialogController {
	...
	public void setCategoryService(CategoryService categoryService, String actionBtnString) {
	...
	public void setCategoryBeanFields(CategoryBean categoryBean, LabelValueBean labelValueBean) {
		this.categoryBean = categoryBean;		
		x_CATEGORY_CODE.setText(categoryBean.getX_CATEGORY_CODE());
		x_CATEGORY_NAME.setText(categoryBean.getX_CATEGORY_NAME());
		x_CATEGORY_DESCRIPTION.setText(categoryBean.getX_CATEGORY_DESCRIPTION());
		x_CATEGORY_TYPE_NAME.setItems(categoryService.getDropdownList());
		x_CATEGORY_TYPE_NAME.getItems().addAll(new LabelValueBean("----(select none)----",null));
		new SelectKeyComboBoxListener(x_CATEGORY_TYPE_NAME); //setting smart search listener
		...
		if ((categoryBean != null) && (categoryBean.getX_STATUS() != null)) {
			...
			x_START_DATE.setValue(CalendarUtil.fromString(categoryBean.getX_START_DATE()));
			x_END_DATE.setValue(CalendarUtil.fromString(categoryBean.getX_END_DATE()));
		}else{
			x_STATUS.setSelected(true);
	...
	@FXML public void handleCategoryStatusAction(){
    ...
    private void handleSubmitUser() throws SQLException {    	
        if (isValidate(actionBtnString)){      
        	...
        	categoryBean.setX_CATEGORY_DESCRIPTION(x_CATEGORY_DESCRIPTION.getText());
        	if(x_CATEGORY_TYPE_NAME.getValue()!=null && !x_CATEGORY_TYPE_NAME.getValue().getLabel().equals("----(select none)----")){
	        	categoryBean.setX_CATEGORY_TYPE_NAME(x_CATEGORY_TYPE_NAME.getValue().getLabel());
	        	categoryBean.setX_CATEGORY_TYPE_ID(x_CATEGORY_TYPE_NAME.getValue().getValue());
	        	categoryBean.setX_COMPANY_ID(x_CATEGORY_TYPE_NAME.getValue().getExtra());
	        	categoryBean.setX_SOURCE_CODE(x_CATEGORY_TYPE_NAME.getValue().getExtra1());
        	}        	
        	categoryBean.setX_STATUS(x_STATUS.isSelected()?"A":"I");
        	if(x_START_DATE.getValue() != null){
        		categoryBean.setX_START_DATE(x_START_DATE.getValue().toString());
        	}else{
        	...
        	if(actionBtnString.equals("search")){
        		categoryMain.refreshCategoryTable(categoryService.getSearchList(categoryBean));	
				okClicked = true;
				dialogStage.close();
	...
	public boolean isValidate(String actionBtnString) {
		if(!actionBtnString.equals("search")){
			String errorMessage = "";
			if (x_CATEGORY_CODE.getText() == null || x_CATEGORY_CODE.getText().length() == 0) {
				errorMessage += "No valid category code!\n"; 
			}
			if (x_CATEGORY_NAME.getText() == null || x_CATEGORY_NAME.getText().length() == 0) {
				errorMessage += "No valid category name!\n"; 
			}		
			if (x_CATEGORY_TYPE_NAME.getValue() == null || x_CATEGORY_TYPE_NAME.getValue().toString().length() == 0
					|| x_CATEGORY_TYPE_NAME.getValue().getLabel().equals("----(select none)----")) {
