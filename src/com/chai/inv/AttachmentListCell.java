...
//	}
	...
	public static HBox getWrappedListData(String txt) {
		...
		button.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				System.out.println("In clear notification button action method");				
				MainApp.notificationPaneListView.getItems().remove(button.getParent());
				...
				if(MainApp.notificationPaneListView.getItems()==null || MainApp.notificationPaneListView.getItems().size()==0){
					MainApp.notificationPaneListView.setPrefHeight(0.0);
					System.out.println("Notification List View PrefHeight when all items are cleared: "+MainApp.notificationPaneListView.getPrefHeight());
					MainApp.notificationPane.hide();
					System.out.println("Notification List View Size when all items are cleared: "+MainApp.notificationPaneListView.getItems().size());
				}
			}
		});
		return hbox;
