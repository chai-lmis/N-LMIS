...
public class MiscellaneousReceiveController implements LotReload {
	@FXML	private ComboBox<LabelValueBean> x_ITEM;
	@FXML	private ComboBox<LabelValueBean> x_SUBINVENTORY;
	@FXML	private ComboBox<LabelValueBean> x_LOCATOR;
	@FXML	private ComboBox<LabelValueBean> x_LOT_NUMBER;
	@FXML	private TextField x_ON_HAND;
	@FXML	private TextField x_QUANTITY;
	@FXML	private TextField x_UOM;
	@FXML	private TextField x_PRICE;
	@FXML	private TextField x_TRANSACTION_NUMBER;
	@FXML	private TextArea x_REASON;
	@FXML	private TableView<OnhandItemQuantity> onhandItemQuantityTable;
	@FXML	private Label x_ITEM_DESCRIPTION;
	@FXML	private Button x_OK_BTN;
	@FXML	private Button x_ADD_LOT_BTN;
	@FXML	private TableColumn<TransactionBean, String> x_LOT_NUMBER_COLUMN;
	@FXML	private TableColumn<TransactionBean, String> x_ONHAND_QUANTITY_COLUMN;
	@FXML	private TableColumn<TransactionBean, String> x_TRANSACTION_UOM_COLUMN;
	@FXML	private TableColumn<TransactionBean, String> x_EXPIRATION_DATE_COLUMN;
	@FXML	private TableColumn<TransactionBean, String> x_WAREHOUSE_NAME_COLUMN;
	@FXML	private TableColumn<TransactionBean, String> x_SUBINVENTORY_CODE_COLUMN;
	@FXML	private TableColumn<TransactionBean, String> x_BIN_LOCATION_CODE_COLUMN;
	...
	@FXML private void changeItemHandle(){
	...
	@FXML private void changeSubinventoryHandle(){
		LabelValueBean labelValueBean = new LabelValueBean();
	...
	@FXML private void changeLocatorHandle(){
		LabelValueBean labelValueBean = new LabelValueBean();
	...
	@FXML private void changeLotHandle(){
		LabelValueBean labelValueBean = new LabelValueBean();
	...
	@FXML private void initialize() {
		...
		x_REASON.textProperty().addListener(new ChangeListener<String>() {
            @Override
            public void changed(ObservableValue<? extends String> observableValue, String s, String s1) {
                if (s1.length()==0 || s1 == null)
                    return;
                if (s1.length()>30) {
                   x_REASON.replaceText(30, s1.length(), "");                 
                }
            }
        });
	}
	...
	@FXML public void handleOK() throws SQLException{
		if(isValid()){
			...
			transactionBean.setX_FROM_SOURCE("MISC");
			...
			if(x_LOCATOR.getValue()!=null){
				transactionBean.setX_TO_BIN_LOCATION_ID(x_LOCATOR.getValue().getValue());
			}			
			transactionBean.setX_FROM_BIN_LOCATION_ID(null);
			transactionBean.setX_TRANSACTION_TYPE_CODE("MISC_RECEIPT");
			if(x_LOT_NUMBER!=null && x_LOT_NUMBER.getValue()!=null){
			...
			if(x_PRICE!=null && x_PRICE.getText().length()!=0){
				transactionBean.setX_UNIT_COST(x_PRICE.getText());
			}
			if(x_REASON!=null && x_REASON.getText().length()!=0){
			...
			if(transactionService.insertMiscReceiveRecord(transactionBean)){
				...
				.message("State Store Receipt Successfully.. ")
				.showInformation();
				dialogStage.close();
	...
	@Override public void reloadLotNumbers(){
		LabelValueBean labelValueBean = new LabelValueBean();
		labelValueBean=x_ITEM.getValue();
		onhandItemQuantity.setX_LOT_NUMBER(null);
		if(labelValueBean != null){
	...
	@FXML public boolean handleLotAddAction() {
