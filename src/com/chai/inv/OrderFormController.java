...
public class OrderFormController {
	...
	@FXML
	private DatePicker x_EXPECTED_DATE;
	@FXML
	private Label x_EXPECTED_DATE_LABEL;
	@FXML
	...
	@FXML
	private BorderPane x_BORDER_PANE;
	@FXML
	private VBox x_BOTTOM_VBOX;
	...
	@FXML
	private TableColumn<AddOrderLineFormBean, String> x_RECEIVED_QTY_COL;
	@FXML
	private TableColumn<AddOrderLineFormBean, String> x_RECEIVED_DATE_COL;
	@FXML
	...
	// declared for AddOrderLineController's OK button visibility(disable or
	// enable)
	private boolean OK_BTN_DISABLE = false;
	@FXML
	private Button x_SUBMIT_BTN;
	@FXML
	...
	@FXML
	private MenuItem x_CONTEXT_MENU_REMOVE_LINE;
	...
	public void setX_SELECTED_STORE_NAME(
					...
					row.setOnMouseClicked(event -> {
						if (event.getClickCount() == 2 && (!row.isEmpty())) {
							...
							if (addOrderLineFormBean == null) {
								...
								System.out
										.println("addOrderLineFormBean found null on double click..now instantiated");
							}
							addOrderLineFormBean = x_ORDER_LINE_ITEMS_TABLE
									.getSelectionModel().getSelectedItem();
							line_Table_action_Btn_String = "edit";
							if (!(actionBtnString.equals("edit")
									&& addOrderLineFormBean.getX_LINE_STATUS()
											.equals("CANCEL") && activeReceiveLineStatus)) {
								...
								Dialogs.create()
										...
										.title("Warning")
										.masthead(
												"Cannot Edit/Receive cancelled product")
										.showWarning();
							}
						}
					});
	...
	@FXML
	public boolean handleOnContextMenuRequested() {
	...
	@FXML
	public boolean handleContextMenuAction() {
		boolean flag = false;
		System.out
				.println("In handleContextMenuAction - menuitem - remove product line");
		AddOrderLineFormBean orderLineBean = x_ORDER_LINE_ITEMS_TABLE
				.getSelectionModel().getSelectedItem();
		if (actionBtnString.equals("add")) {
			if (orderLineBean != null) {
				...
				Dialogs.create()
						...
						.message(
								"Please select a product line from table to remove it.")
						.showWarning();
			}
		} else {
			x_CONTEXT_MENU_REMOVE_LINE.setDisable(true);
	...
	public void callSceneSizeListener() {
		dialogStage.getScene().widthProperty()
		...
		dialogStage.getScene().heightProperty()
				.addListener(new ChangeListener<Number>() {
					@Override
					public void changed(
							ObservableValue<? extends Number> observableValue,
	...
	public void refreshOrderItemLineTable(
		...
		System.out
				.println("In OrderFormController.refreshOrderItemLineTable(orderformbean) method");
		x_ORDER_LINE_ITEMS_TABLE.setItems(list);
	}

	public void setFormDefaults(OrderFormBean orderFormBean,
		...
		if (actionBtnString.equals("search")) {
			...
			String order_number = orderFormService
					.getAutoGenerateOrderNumber(true);
			if (order_number == null) {
				...
				x_ORDER_NUMBER.setText(order_number);
		}
		x_ORDER_DATE.setValue(LocalDate.now());
		x_ORDER_FROM.getItems().addAll(new LabelValueBean("Store", "2"),
				new LabelValueBean("----(select none)----", ""));
		...
		LabelValueBean defaultOrderingStoreBean = orderFormService
				.getDefaultOrderingStore(userBean.getX_USER_WAREHOUSE_ID());
		if (defaultOrderingStoreBean.getExtra() != null
				&& defaultOrderingStoreBean.getExtra1() != null) {
			System.out.println("defaultOrderingStoreBean.getExtra() : "
					+ defaultOrderingStoreBean.getExtra());
			System.out.println("defaultOrderingStoreBean.getExtra1() : "
					+ defaultOrderingStoreBean.getExtra1());
			x_STORE_TYPE.setValue(new LabelValueBean(defaultOrderingStoreBean
					.getExtra(), defaultOrderingStoreBean.getExtra1()));
		} else {
			x_STORE_TYPE.setValue(orderFormService.getDropdownList("StoreType")
					.get(0));
			System.out
					.println("orderFormService.getDropdownList('StoreType').get(0) : "
							+ orderFormService.getDropdownList("StoreType")
									.get(0).getLabel());
		}
		if (defaultOrderingStoreBean.getLabel() != null
				&& defaultOrderingStoreBean.getValue() != null) {
			x_SELECTED_STORE_NAME.setValue(defaultOrderingStoreBean);
		}
		ObservableList<LabelValueBean> PO_Order_Status_List = orderFormService
				.getDropdownList("POOrderStatus");
		for (LabelValueBean lvb : PO_Order_Status_List) {
			System.out
					.println("In PO order status list loop..................");
			if (lvb.getLabel().equals("OPEN")) {
				System.out
						.println("trueeeeee-------------------------------- OPEN");
		...
		if (orderFormBean != null) {
			if (activeReceiveLineStatus
					&& orderFormBean.getX_ORDER_STATUS() != null
					&& orderFormBean.getX_ORDER_STATUS().toUpperCase()
							.equals("SUBMITTED")) {
				order_submitted_already = true;
				System.out
						.println("==============In activeRceive = true && status=submitted===========");
				x_SAVE_BTN.setDisable(true);
				x_ADD_LINE_ITEM_BTN.setDisable(true);
				setOK_BTN_DISABLE(true);
				if (orderFormBean.getX_ORDER_STATUS().toUpperCase()
						.equals("CANCEL")) {
					order_cancel_already = true;
					order_cancel_already2 = false;
					System.out.println("***order_cncel_already is true");
					System.out
							.println("***orderFormBean.getX_ORDER_STATUS().toUpperCase() : "
									+ orderFormBean.getX_ORDER_STATUS()
											.toUpperCase());
				}
				order_cancel_already = true; // set true manually to avoid
												// complete order cancel pop-up
				x_ORDER_STATUS.getItems().remove(ORDER_STATUS_OPEN_LVB);
				order_cancel_already = false; // set true manually to avoid
												// complete order cancel pop-up
			}
			if (activeReceiveLineStatus == false
					&& orderFormBean.getX_ORDER_STATUS() != null
					&& orderFormBean.getX_ORDER_STATUS().toUpperCase()
							.equals("SUBMITTED")) {
				System.out
						.println("==============In activeRceive = false && status=submitted===========");
				order_submitted_already = true;
				x_SAVE_BTN.setDisable(true);
				x_ADD_LINE_ITEM_BTN.setDisable(true);
				setOK_BTN_DISABLE(true);
				...
				order_cancel_already = false;// set true manually to avoid
												// complete order cancel pop-up
				System.out.println("2.x_ORDER_STATUS = "
						+ x_ORDER_STATUS.getValue().getLabel());
			} else {
				setOK_BTN_DISABLE(false);
			}
			if (orderFormBean != null
					&& orderFormBean.getX_RECEIVED_DATE() != null) {
				x_RECEIVED_QTY_COL.setVisible(true);
				x_RECEIVED_DATE_COL.setVisible(true);
				// activeReceiveLineStatus = true;
			} else {
			...
			if (orderFormBean.getX_ORDER_STATUS() != null
					&& orderFormBean.getX_ORDER_STATUS().toUpperCase()
							.equals("RECEIVED")) {
				order_cancel_already = true; // set true manually to avoid
												// complete order cancel pop-up
				...
				order_cancel_already = false; // set true manually to avoid
												// complete order cancel pop-up
				System.out.println("x_ORDER_STATUS = "
						+ x_ORDER_STATUS.getValue().getLabel());
			}
			if (orderFormBean.getX_ORDER_STATUS() != null
					&& orderFormBean.getX_ORDER_STATUS().toUpperCase()
			...
			list = orderFormService.getOrderLineList(labelValueBean.getLabel(),
					order_cancel_already);
			x_ORDER_LINE_ITEMS_TABLE.setItems(list);
			...
			x_ORDER_DATE.setValue(CalendarUtil.fromString(orderFormBean
					.getX_ORDER_DATE()));
			x_ORDER_FROM.setValue(new LabelValueBean(orderFormBean
					.getX_ORDER_FROM_SOURCE(), "2"));
			// x_STORE_TYPE.setValue(new
			// LabelValueBean(orderFormBean.getX_ORDER_TO_SOURCE_TYPE_NAME(),
			// orderFormBean.getX_ORDER_TO_SOURCE_TYPE_ID()));
			x_STORE_TYPE.setValue(new LabelValueBean(orderFormBean
					.getX_ORDER_FROM_SOURCE_TYPE_NAME(), orderFormBean
					.getX_ORDER_FROM_SOURCE_TYPE_ID()));
			x_SELECTED_STORE_NAME
					.setValue(new LabelValueBean(orderFormBean
							.getX_ORDER_FROM_NAME(), orderFormBean
							.getX_ORDER_FROM_ID()));// DB
			x_EXPECTED_DATE.setValue(CalendarUtil.fromString(orderFormBean
					.getX_EXPECTED_DATE()));
			x_ORDER_STATUS
					.setValue(new LabelValueBean(orderFormBean
							.getX_ORDER_STATUS(), orderFormBean
							.getX_ORDER_STATUS_ID()));
	...
	public void handleOrderToChange() {
		...
		switch (lbv.getValue()) {
		case "1":
			x_STORE_TYPE.setItems(orderFormService.getDropdownList("Customer"));
	...
	public void handleStoreTypeChange() {
		System.out
				.println("In OrderFormController.handleStoreTypeChange() handler");
		LabelValueBean lbv = new LabelValueBean();
		...
		default:
			x_SELECTED_STORE_NAME
					.setPromptText("Select option from Store-Type list");
		}
	}
	...
	public void handleSubmitBtnAction() throws Exception {
		...
		System.out.println("order already submitted : "
				+ order_submitted_already);
		if ((order_status.equals("OPEN") || actionBtnString.equals("add") || actionBtnString
				.equals("edit")) && !order_submitted_already) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				System.out
						.println("X_LINE_STATUS : " + aolb.getX_LINE_STATUS());
				if (aolb.getX_LINE_STATUS().equals("OPEN")) {
					if (ORDER_STATUS_SUBMITTED_LVB == null) {
						System.out
								.println("ORDER_STATUS_SUBMITTED_LVB is nullllllllllllll");
					}
					if (ORDER_STATUS_SUBMITTED_LVB.getLabel() == null) {
						System.out
								.println("ORDER_STATUS_SUBMITTED_LVB label is nullllllllllllll");
					}
					if (ORDER_STATUS_SUBMITTED_LVB.getValue() == null) {
						System.out
								.println("ORDER_STATUS_SUBMITTED_LVB value is nullllllllllllll");
					}
					aolb.setX_LINE_STATUS(x_ORDER_STATUS.getValue().getLabel());
					aolb.setX_LINE_STATUS_ID(x_ORDER_STATUS.getValue()
							.getValue());
					System.out.println("aolb.getX_ORDER_LINE_ID: " + i + "-->"
							+ aolb.getX_ORDER_LINE_ID());
					list.set(i, aolb);
				}
			}
			System.out
					.println("X_LINE_STATUS : After setting the line status in list");
			for (AddOrderLineFormBean aolb : list) {
				System.out.println("------X_LINE_STATUS : "
						+ aolb.getX_LINE_STATUS());
				System.out
						.println("In submitBtnAction------X_LINE_CANCEL_DATE_2: "
								+ aolb.getX_LINE_CANCEL_DATE_2());
				System.out
						.println("In submitBtnAction------X_LINE_CANCEL_DATE: "
								+ aolb.getX_LINE_CANCEL_DATE());
			}
			handleSubmitOrders();
		}
	}

	public void setStatusCancelOnOrderAndSave(LabelValueBean lvb)
			throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			aolb.setX_LINE_CANCEL_DATE(CalendarUtil.toDateString(LocalDate
					.now()));
			aolb.setX_LINE_CANCEL_DATE_2(LocalDate.now().toString());
			aolb.setX_LINE_CANCEL_REASON("Line Item cancelled by user: "
					+ userBean.getX_FIRST_NAME() + " "
					+ userBean.getX_LAST_NAME());
			System.out.println("aolb.getX_ORDER_LINE_ID: " + i + "-->"
					+ aolb.getX_ORDER_LINE_ID());
			System.out.println("aolb.getX_LINE_ITEM_ID : "
					+ aolb.getX_LINE_ITEM_ID());
			list.set(i, aolb);
		}
		for (AddOrderLineFormBean aolb : list) {
			System.out
					.println("When complete order is cancelled... line items data is as given below: PO");
			System.out.println("aolb.getX_LINE_ITEM_ID : "
					+ aolb.getX_LINE_ITEM_ID());
			System.out.println("aolb.getX_LINE_CANCEL_DATE(): "
					+ aolb.getX_LINE_CANCEL_DATE());
			System.out.println("aolb.getX_LINE_CANCEL_DATE_2(): "
					+ aolb.getX_LINE_CANCEL_DATE_2());
			System.out.println("aolb.getX_LINE_CANCEL_REASON(): "
					+ aolb.getX_LINE_CANCEL_REASON() + "\n\n");
		}
		handleSubmitOrders();
	}
	...
	public void handleOrderStatusChange() throws Exception {
		...
		if (actionBtnString.equals("edit")) {
			if (lbv.getLabel().equals("CANCEL") && !order_cancel_already) {
				...
				Action response = Dialogs
						...
						.message(
								"click Ok to confirm cancellation, else click Cancel")
				...
				if (response == Dialog.Actions.OK) {
					...
					x_CANCEL_REASON
							.setText("cancelled the complete order by user: "
									+ userBean.getX_FIRST_NAME() + " "
									+ userBean.getX_LAST_NAME());
					setStatusCancelOnOrderAndSave(lbv);
				} else {
					System.out.println("size : "
							+ x_ORDER_STATUS.getItems().size());
					x_ORDER_STATUS.setValue(new LabelValueBean(
							ORDER_STATUS_OPEN_LVB.getLabel(),
							ORDER_STATUS_OPEN_LVB.getValue()));
				...
				x_ADD_LINE_ITEM_BTN.setDisable(true);
				setOK_BTN_DISABLE(true);
				if (order_cancel_already && lbv.getLabel().equals("CANCEL")
						&& order_cancel_already2) {
					System.out.println("order_status : disable - "
							+ order_cancel_already);
		...
		if (lbv.getLabel() != null && lbv.getLabel().equals("SUBMITTED")) {
			if (activeReceiveLineStatus && actionBtnString.equals("edit")) {
				System.out
						.println("===========Status change=edit==In activeReceive = true && status=submitted===========");
				x_SAVE_BTN.setDisable(true);
				x_ADD_LINE_ITEM_BTN.setDisable(true);
				setOK_BTN_DISABLE(true);
			} else if (activeReceiveLineStatus == false
					&& actionBtnString.equals("edit")) {
				System.out
						.println("======Status change== actionBtn=edit =====In activeReceive = false && status=submitted===========");
				...
				System.out
						.println("======Status change========In actionBtn=add && activeReceive = false && status=submitted===========");
				x_SAVE_BTN.setDisable(false);
				x_ADD_LINE_ITEM_BTN.setDisable(false);
				setOK_BTN_DISABLE(false);
			}
	...
	public void handleSubmitOrders() throws Exception {
		if (isValidate(actionBtnString)) {
			...
			orderFormBean.setX_ORDER_TYPE("Purchase Order"); // for now
																// hardcoded
			if (x_ORDER_NUMBER != null
					&& x_ORDER_NUMBER.getText().length() != 0) {
				orderFormBean
						.setX_ORDER_HEADER_NUMBER(x_ORDER_NUMBER.getText());
			}
			if (!x_ORDER_FROM.getValue().getLabel()
					.equals("----(select none)----")) {
				...
				orderFormBean.setX_ORDER_FROM_SOURCE_TYPE_ID(x_STORE_TYPE
						.getValue().getValue());
			}
			...
			if (x_EXPECTED_DATE != null && x_EXPECTED_DATE.getValue() != null) {
				orderFormBean.setX_EXPECTED_DATE(x_EXPECTED_DATE.getValue()
						.toString());
			}
			if (!x_SELECTED_STORE_NAME.getValue().getLabel()
					.equals("----(select none)----")) {
				orderFormBean.setX_ORDER_FROM_ID(x_SELECTED_STORE_NAME
						.getValue().getValue());
				orderFormBean.setX_ORDER_FROM_NAME(x_SELECTED_STORE_NAME
						.getValue().getLabel());
				orderFormBean.setX_ORDER_FROM_SOURCE_TYPE_NAME(x_STORE_TYPE
						.getValue().getLabel());
			}
			if (!x_ORDER_STATUS.getValue().getLabel()
					.equals("----(select none)----")) {
				orderFormBean.setX_ORDER_STATUS_ID(x_ORDER_STATUS.getValue()
						.getValue());
				orderFormBean.setX_ORDER_STATUS(x_ORDER_STATUS.getValue()
						.getLabel());
			}
			if (x_ORDER_DATE != null && x_ORDER_DATE.getValue() != null) {
				orderFormBean.setX_ORDER_DATE(x_ORDER_DATE.getValue()
						.toString());
			}
			// if-else is used to avoid '' empty string, which fails search
			// results.
			if (x_ORDER_STATUS.getValue() != null
					&& x_ORDER_STATUS.getValue().getLabel()
							.equalsIgnoreCase("CANCEL")) {
				if (x_CANCEL_DATE != null && x_CANCEL_DATE.getValue() != null) {
					orderFormBean.setX_CANCEL_DATE(x_CANCEL_DATE.getValue()
							.toString());
				}
			...
			orderFormBean.setX_ORDER_HEADER_ID(userBean
					.getX_USER_WAREHOUSE_ID()
					+ orderFormBean.getX_ORDER_HEADER_NUMBER()
					+ orderFormBean.getX_ORDER_TYPE_ID());
			System.out.println("concatenated order_header_id : "
					+ orderFormBean.getX_ORDER_HEADER_ID());

			if (actionBtnString.equals("search")) {
				orderMain.refreshOrderTable(orderFormService.getSearchList(
						orderFormBean, "Purchase Order"));
				okClicked = true;
				dialogStage.close();
				DatabaseOperation.getDbo().closeConnection();
				DatabaseOperation.setDbo(null);
				...
				if (OrderStatusValidation.validateOrderStatus(actionBtnString,
						orderFormBean.getX_ORDER_STATUS(), list, dialogStage,
						this)) {
					// (actionBtnString.equals("edit"))?orderFormBean.getX_ORDER_HEADER_ID():orderHeaderID
					orderLineInsertSuccess = orderFormService
							.saveOrderLineItems(list, actionBtnString,
									orderFormBean.getX_ORDER_HEADER_ID(),
									orderFormBean.getX_ORDER_TO_ID(),
									orderFormBean.getX_ORDER_FROM_ID());
					if (order_open_already && actionBtnString.equals("edit")) {
						System.out
								.println("In if block, order_open_already && actionBtnString.equals('edit')..............");
						AddOrderLineFormBean lineFormBean = new AddOrderLineFormBean();
						ObservableList<AddOrderLineFormBean> lineInsertList = FXCollections
								.observableArrayList();
						for (int i = 0; i < list.size(); i++) {
							if (list != null) {
								...
								if (list.get(i) != null) {
									System.out
											.println("in if block list.get(i)!=null");
									if (list.get(i).getX_OPERATION_ON_BEAN() != null) {
										System.out
												.println("in if block list.get(i).getX_OPERATION_ON_BEAN()!=null");
										if (list.get(i)
												.getX_OPERATION_ON_BEAN()
												.equals("NEW_INSERT")) {
											System.out
													.println("in if block list.get(i).getX_OPERATION_ON_BEAN()=='new_insert'");
											lineFormBean = list.get(i);
											lineInsertList.add(lineFormBean);
											System.out
													.println("lineInsertList.size(): "
															+ lineInsertList
																	.size());
										}
									}
						...
						if (lineInsertList.size() != 0) {
							System.out
									.println("++++++++++++size of insertList!=0++++++++++++++++++++++++++++++++++++++++++++++++++");
							boolean lineInsertListSuccess = orderFormService
									.saveOrderLineItems(lineInsertList, "add",
											orderFormBean
													.getX_ORDER_HEADER_ID(),
											orderFormBean.getX_ORDER_TO_ID(),
											orderFormBean.getX_ORDER_FROM_ID());
							System.out.println("lineInsertListSuccess = "
									+ lineInsertListSuccess);
						}
					}
				}
				if (orderLineInsertSuccess) {
					orderHeaderInsertSuccess = orderFormService
							.saveOrderHeaders(orderFormBean, actionBtnString);
					System.out
							.println("returned back from saveOrderheaders()****************************");
				}
				System.out
						.println("orderFormBean.getX_ORDER_HEADER_ID() ========= "
								+ orderFormBean.getX_ORDER_HEADER_ID());
				try {
					if (!DatabaseOperation.getDbo().getConnection()
							.getAutoCommit()) {
				...
				if (orderLineInsertSuccess && orderHeaderInsertSuccess) {
					...
					System.out
							.println("------------order data insert is not successful------- ");
				}
				DatabaseOperation.getDbo().closeConnection();
				DatabaseOperation.setDbo(null);
			}
	...
	public boolean isValidate(String actionBtnString) {
		if (!actionBtnString.equals("search")) {
			...
			if (x_EXPECTED_DATE.getValue() == null
					|| x_EXPECTED_DATE.getValue().toString().length() == 0) {
				errorMessage += "No valid expected date!\n";
			}
			if (x_ORDER_FROM.getValue() == null
					|| x_ORDER_FROM.getValue().toString().length() == 0
					|| x_ORDER_FROM.getValue().getLabel()
							.equals("----(select none)----")) {
			...
			if (x_ORDER_LINE_ITEMS_TABLE == null
					|| x_ORDER_LINE_ITEMS_TABLE.getItems().isEmpty()) {
	...
	public boolean handleOpenOrderLineForm() {
		...
		try {
			...
			controller.setOrderLineItemGridList(list, actionBtnString,
					orderFormBean.getX_ORDER_HEADER_ID());
			...
			if (line_Table_action_Btn_String.equals("edit")) {
				if (x_ORDER_STATUS.getValue() == null) {
					System.out
							.println("OrderFormcontroller : x_ORDER_STATUS is null when double click.");
				}
				System.out.println("SHIPPED QUANTITY IN STOCK ORDER FORM: "
						+ addOrderLineFormBean.getX_LINE_SHIP_QTY());
				if (addOrderLineFormBean == null) {
					System.out
							.println("addOrderLineFormBean is null in handleOpenOrderLineForm : double click");
				} else {
					System.out
							.println("addOrderLineFormBean is not null in handleOpenOrderLineForm : "
